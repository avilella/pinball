...
my ($readsfile,$phred64,$no_permute,$dust,$sample,$readsfile,$onlyindex,$minreadlen,$tag,$exhaustive_overlap,$overlap,$csize,$threads,$debug);
my $disk = 1000000;
$threads = 1;
$overlap = 31;
$csize = 50;
...
# sga preprocess
$cmd = "$sga_executable preprocess $sample_threshold $phred64_flag --min-length=$minreadlen $dust_threshold $permute_ambiguous $readsfile -o $tag.fq 2>$tag.preprocess.txt";
print STDERR "$cmd\n" if ($debug);
$rerun_string .= "$cmd\n";
